; Mutation control: a throwing target skips trace/end!.
(set-logic ALL)
...
(assert (= actual-target-calls expected-target-calls))
(assert (= actual-spans-started expected-spans-started))
(assert (= actual-spans-ended
           (ite (and (not suppressed) known-operation (not target-throws)) 1 0)))
(assert (= actual-error-status expected-error-status))
(assert (= actual-return-identity expected-return-identity))
(assert (= actual-throw-identity expected-throw-identity))
(assert (= actual-invalid-rejection expected-invalid-rejection))
(assert (= violation
...
(assert (! violation :named missing_finally_detected))
